...
SERVER.send_access_token = function(res, user_id, client_id, client_data){
	 
       var access_token = oauth2.create_access_token(user_id, client_id, "")
        // optional: expires_in, refresh_token, scope
    
      redirect(res, client_data.redirect_uri + '?access_token=' + access_token+'&uid=' + user_id);
...
SERVER.authorize = function(params, req, res) {
  ...
  Client.getById(params.client_id, function(err, client) {
//	console.log("params:"+util.inspect(params, false, 1));
//	console.log("client:"+util.inspect(client, false, 1));
    if (err)  return unknown_error(res, err);
...
//	console.log("client redirect uri:"+ client.get('redirect_uri') );
...
exports.connector = function(config, Model, authentication) {
 ...
 console.log(util.inspect(config, false, 1));
  return router(function(app) {
    ...
    app.get(config.failure_url, authentication.failed_login);
    app.get(config.process_logout_url, authentication.logout);
  });
};
